...
- (void)directFlag:(CGContextRef)ctx rect:(CGRect)rect {
    ...
    CGFloat topY =
        rect.origin.y + (rect.size.width * .5f - h - marginSelectedCirclev -
                         self.selectedRect.size.height * .5f);
...
- (void)propertySetting:(CGContextRef)ctx {
    ...
    UIColor *color = nil;
    if (_selected) {
        color = GestureLockCircleLineSelectedColor;
        //设置线宽
        CGContextSetLineWidth(ctx, GestureLockSelectedCircleLineWidth);
    } else {
        color = GestureLockCircleLineNormalColor;
        //设置线宽
...
- (CGRect)calRect {
    if (CGRectEqualToRect(_calRect, CGRectZero)) {
        CGFloat lineW = GestureLockSelectedCircleLineWidth;
        CGFloat sizeWH = self.bounds.size.width - lineW;
        CGFloat originXY = lineW * .5f;
        //添加一个圆环路径
        _calRect = (CGRect){CGPointMake(originXY, originXY),
                            CGSizeMake(sizeWH, sizeWH)};
    }
    return _calRect;
}
...
- (CGRect)selectedRect {
    if (CGRectEqualToRect(_selectedRect, CGRectZero)) {
        ...
        CGFloat selectRectWH =
            rect.size.width * GestureLockSelectedCircleProportion;
        CGFloat selectRectXY =
            rect.size.width * (1 - GestureLockSelectedCircleProportion) * .5f;
        _selectedRect =
            CGRectMake(selectRectXY, selectRectXY, selectRectWH, selectRectWH);
    }
